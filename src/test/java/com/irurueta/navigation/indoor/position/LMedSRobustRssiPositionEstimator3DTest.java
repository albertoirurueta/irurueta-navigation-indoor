...
class LMedSRobustRssiPositionEstimator3DTest implements RobustRssiPositionEstimatorListener<Point3D> {

    private static final Logger LOGGER = Logger.getLogger(LMedSRobustRssiPositionEstimator3DTest.class.getName());
    ...
    void testConstructor() {
        // empty constructor
        var estimator = new LMedSRobustRssiPositionEstimator3D();

        // check default values
        assertEquals(LMedSRobustLateration3DSolver.DEFAULT_STOP_THRESHOLD, estimator.getStopThreshold(), 0.0);
        ...
        estimator = new LMedSRobustRssiPositionEstimator3D(sources);
        ...
        assertThrows(IllegalArgumentException.class,
                () -> new LMedSRobustRssiPositionEstimator3D((List<WifiAccessPointLocated3D>) null));
        final var wrongSources = new ArrayList<WifiAccessPointLocated3D>();
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator3D(wrongSources));

        // constructor with fingerprints
        final var fingerprint = new RssiFingerprint<WifiAccessPoint, RssiReading<WifiAccessPoint>>();
        estimator = new LMedSRobustRssiPositionEstimator3D(fingerprint);
        ...
        // force IllegalArgumentException
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator3D(
                (RssiFingerprint<WifiAccessPoint, RssiReading<WifiAccessPoint>>) null));

        // constructor with sources and fingerprint
        estimator = new LMedSRobustRssiPositionEstimator3D(sources, fingerprint);
        ...
        // force IllegalArgumentException
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator3D(null,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator3D(wrongSources,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator3D(sources,
                (RssiFingerprint<WifiAccessPoint, RssiReading<WifiAccessPoint>>) null));

        // constructor with listener
        estimator = new LMedSRobustRssiPositionEstimator3D(this);
        ...
        estimator = new LMedSRobustRssiPositionEstimator3D(sources, this);
        ...
        estimator = new LMedSRobustRssiPositionEstimator3D(fingerprint, this);
        ...
        estimator = new LMedSRobustRssiPositionEstimator3D(sources, fingerprint, this);
    ...
    void testGetSetStopThreshold() throws LockedException {
        final var estimator = new LMedSRobustRssiPositionEstimator3D();
    ...
    void testGetSetPreliminarySubsetSize() throws LockedException {
        final var estimator = spy(new LMedSRobustRssiPositionEstimator3D());
    ...
    void testEstimate() throws LockedException, RobustEstimatorException, NotReadyException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (var i = 0; i < numSources; i++) {
                ...
                readings.add(new RssiReading<>(accessPoint, rssi + error, Math.sqrt(RX_POWER_VARIANCE)));
            }

            final var fingerprint = new RssiFingerprint<>(readings);

            final var estimator = new LMedSRobustRssiPositionEstimator3D(sources, fingerprint, this);
    ...
    public void onEstimateStart(final RobustRssiPositionEstimator<Point3D> estimator) {
        estimateStart++;
        checkLocked((LMedSRobustRssiPositionEstimator3D) estimator);
    }
    ...
    private static double receivedPower(final double equivalentTransmittedPower, final double distance,
        ...
        final var k = Math.pow(SPEED_OF_LIGHT / (4.0 * Math.PI * LMedSRobustRssiPositionEstimator3DTest.FREQUENCY),
                pathLossExponent);
        return equivalentTransmittedPower * k / Math.pow(distance, pathLossExponent);
    }

    private static void checkLocked(final LMedSRobustRssiPositionEstimator3D estimator) {
