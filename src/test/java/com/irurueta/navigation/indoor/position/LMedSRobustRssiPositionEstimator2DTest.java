...
class LMedSRobustRssiPositionEstimator2DTest implements RobustRssiPositionEstimatorListener<Point2D> {

    private static final Logger LOGGER = Logger.getLogger(LMedSRobustRssiPositionEstimator2DTest.class.getName());
    ...
    void testConstructor() {
        // empty constructor
        var estimator = new LMedSRobustRssiPositionEstimator2D();
        ...
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator2D(
                (List<WifiAccessPointLocated2D>) null));
        final var wrongSources = new ArrayList<WifiAccessPointLocated2D>();
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator2D(wrongSources));
        ...
        // force IllegalArgumentException
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator2D(null,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator2D(wrongSources,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new LMedSRobustRssiPositionEstimator2D(sources,
                (RssiFingerprint<WifiAccessPoint, RssiReading<WifiAccessPoint>>) null));
        ...
        estimator = new LMedSRobustRssiPositionEstimator2D(sources, fingerprint, this);
    ...
    void testGetSetStopThreshold() throws LockedException {
        final var estimator = new LMedSRobustRssiPositionEstimator2D();
    ...
    void testGetSetPreliminarySubsetSize() throws LockedException {
        final var estimator = spy(new LMedSRobustRssiPositionEstimator2D());
    ...
    void testEstimate() throws LockedException, RobustEstimatorException, NotReadyException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var estimator = new LMedSRobustRssiPositionEstimator2D(sources, fingerprint, this);
    ...
    private static double receivedPower(final double equivalentTransmittedPower, final double distance,
        ...
        final var k = Math.pow(SPEED_OF_LIGHT / (4.0 * Math.PI * LMedSRobustRssiPositionEstimator2DTest.FREQUENCY),
                pathLossExponent);
        return equivalentTransmittedPower * k / Math.pow(distance, pathLossExponent);
    }

    private static void checkLocked(final LMedSRobustRssiPositionEstimator2D estimator) {
