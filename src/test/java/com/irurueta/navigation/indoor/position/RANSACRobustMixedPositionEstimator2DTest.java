...
class RANSACRobustMixedPositionEstimator2DTest implements RobustMixedPositionEstimatorListener<Point2D> {

    private static final Logger LOGGER = Logger.getLogger(RANSACRobustMixedPositionEstimator2DTest.class.getName());
    ...
    void testConstructor() {
        // empty constructor
        var estimator = new RANSACRobustMixedPositionEstimator2D();

        // check default values
        assertEquals(RANSACRobustLateration2DSolver.DEFAULT_THRESHOLD, estimator.getThreshold(), 0.0);
        ...
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustMixedPositionEstimator2D(
                (List<WifiAccessPointLocated2D>) null));
        final var wrongSources = new ArrayList<WifiAccessPointLocated2D>();
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustMixedPositionEstimator2D(wrongSources));
        ...
        // force IllegalArgumentException
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustMixedPositionEstimator2D(null,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustMixedPositionEstimator2D(wrongSources,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustMixedPositionEstimator2D(sources,
                (Fingerprint<WifiAccessPoint, Reading<WifiAccessPoint>>) null));
        ...
        estimator = new RANSACRobustMixedPositionEstimator2D(sources, this);
        ...
        estimator = new RANSACRobustMixedPositionEstimator2D(fingerprint, this);
        ...
        estimator = new RANSACRobustMixedPositionEstimator2D(sources, fingerprint, this);
    ...
    void testGetSetThreshold() throws LockedException {
        final var estimator = new RANSACRobustMixedPositionEstimator2D();
    ...
    void testGetSetInitialPosition() throws LockedException {
        final var solver = new RANSACRobustMixedPositionEstimator2D();
    ...
    void testGetSetPreliminarySubsetSize() throws LockedException {
        final var estimator = spy(new RANSACRobustMixedPositionEstimator2D());
    ...
    void testEstimate() throws LockedException, RobustEstimatorException, NotReadyException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var estimator = new RANSACRobustMixedPositionEstimator2D(sources, fingerprint, this);
    ...
    @Test
    void testEstimateLinearSolverUsedInhomogeneousPreliminaryRefined() throws LockedException,
            RobustEstimatorException, NotReadyException, NonSymmetricPositiveDefiniteMatrixException {
        final var randomizer = new UniformRandomizer();
        final var errorRandomizer = new GaussianRandomizer(0.0, STD_OUTLIER_ERROR);
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var fingerprint = new Fingerprint<WifiAccessPoint, Reading<WifiAccessPoint>>(readings);
    ...
    public void onEstimateStart(final RobustMixedPositionEstimator<Point2D> estimator) {
        estimateStart++;
        checkLocked((RANSACRobustMixedPositionEstimator2D) estimator);
    }
    ...
    private final double receivedPower(final double equivalentTransmittedPower, final double distance,
                                       final double pathLossExponent) {
        // Pr = Pt*Gt*Gr*lambda^2/(4*pi*d)^2,    where Pr is the received power
        // lambda = c/f, where lambda is wavelength,
        // Pte = Pt*Gt*Gr, is the equivalent transmitted power, Gt is the transmitted Gain and Gr is the received Gain
        // Pr = Pte*c^2/((4*pi*f)^2 * d^2)
        final var k = Math.pow(SPEED_OF_LIGHT / (4.0 * Math.PI * RANSACRobustMixedPositionEstimator2DTest.FREQUENCY),
                pathLossExponent);
        return equivalentTransmittedPower * k / Math.pow(distance, pathLossExponent);
    }

    private static void checkLocked(final RANSACRobustMixedPositionEstimator2D estimator) {
