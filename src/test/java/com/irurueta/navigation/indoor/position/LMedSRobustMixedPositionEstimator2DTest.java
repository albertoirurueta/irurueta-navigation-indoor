...
public class LMedSRobustMixedPositionEstimator2DTest implements
    ...
    public void testConstructor() {
        // empty constructor
        LMedSRobustMixedPositionEstimator2D estimator =
                new LMedSRobustMixedPositionEstimator2D();
        ...
        estimator = null;
        try {
            estimator = new LMedSRobustMixedPositionEstimator2D(
        ...
        estimator = null;
        try {
            estimator = new LMedSRobustMixedPositionEstimator2D(null, fingerprint);
            fail("IllegalArgumentException expected but not thrown");
        } catch (final IllegalArgumentException ignore) {
        }
        try {
            estimator = new LMedSRobustMixedPositionEstimator2D(new ArrayList<WifiAccessPointLocated2D>(),
                    fingerprint);
            fail("IllegalArgumentException expected but not thrown");
        } catch (final IllegalArgumentException ignore) {
        }
        ...
        // check default values
        assertEquals(LMedSRobustLateration2DSolver.DEFAULT_STOP_THRESHOLD, estimator.getStopThreshold(),
                0.0);
        assertEquals(3, estimator.getMinRequiredSources());
        assertEquals(3, estimator.getPreliminarySubsetSize());
        assertNull(estimator.getSources());
        ...
        estimator = null;
        try {
            estimator = new LMedSRobustMixedPositionEstimator2D(null, fingerprint, this);
            fail("IllegalArgumentException expected but not thrown");
        } catch (final IllegalArgumentException ignore) {
        }
        try {
        ...
        try {
            estimator = new LMedSRobustMixedPositionEstimator2D(sources, null, this);
            fail("IllegalArgumentException expected but not thrown");
        } catch (final IllegalArgumentException ignore) {
        }
        assertNull(estimator);
    ...
    public void testGetSetStopThreshold() throws LockedException {
        final LMedSRobustMixedPositionEstimator2D estimator = new LMedSRobustMixedPositionEstimator2D();
    ...
    public void testGetSetPreliminarySubsetSize() throws LockedException {
        final LMedSRobustMixedPositionEstimator2D estimator =
                spy(new LMedSRobustMixedPositionEstimator2D());
    ...
    public void testEstimate() throws LockedException, RobustEstimatorException, NotReadyException,
        ...
        for (int t = 0; t < TIMES; t++) {
            ...
            final LMedSRobustMixedPositionEstimator2D estimator =
                    new LMedSRobustMixedPositionEstimator2D(sources, fingerprint, this);
    ...
    private double receivedPower(final double equivalentTransmittedPower, final double distance,
        ...
        final double k = Math.pow(SPEED_OF_LIGHT / (4.0 * Math.PI *
                        LMedSRobustMixedPositionEstimator2DTest.FREQUENCY),
                pathLossExponent);
        return equivalentTransmittedPower * k / Math.pow(distance, pathLossExponent);
    }

    private void checkLocked(final LMedSRobustMixedPositionEstimator2D estimator) {
        try {
