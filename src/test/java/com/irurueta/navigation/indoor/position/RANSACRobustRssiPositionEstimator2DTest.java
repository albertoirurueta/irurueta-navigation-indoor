...
class RANSACRobustRssiPositionEstimator2DTest implements RobustRssiPositionEstimatorListener<Point2D> {

    private static final Logger LOGGER = Logger.getLogger(RANSACRobustRssiPositionEstimator2DTest.class.getName());
    ...
    void testConstructor() {
        // empty constructor
        var estimator = new RANSACRobustRssiPositionEstimator2D();
        ...
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustRssiPositionEstimator2D(
                (List<WifiAccessPointLocated2D>) null));
        final var wrongSources = new ArrayList<WifiAccessPointLocated2D>();
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustRssiPositionEstimator2D(wrongSources));
        ...
        // force IllegalArgumentException
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustRssiPositionEstimator2D(null,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustRssiPositionEstimator2D(wrongSources,
                fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new RANSACRobustRssiPositionEstimator2D(sources,
                (RssiFingerprint<WifiAccessPoint, RssiReading<WifiAccessPoint>>) null));
        ...
        estimator = new RANSACRobustRssiPositionEstimator2D(sources, fingerprint, this);
    ...
    void testGetSetThreshold() throws LockedException {
        final var estimator = new RANSACRobustRssiPositionEstimator2D();
    ...
    void testGetSetInitialPosition() throws LockedException {
        final var solver = new RANSACRobustRssiPositionEstimator2D();
    ...
    void testGetSetPreliminarySubsetSize() throws LockedException {
        final var estimator = spy(new RANSACRobustRssiPositionEstimator2D());
    ...
    void testEstimate() throws LockedException, RobustEstimatorException, NotReadyException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (var i = 0; i < numSources; i++) {
                ...
                var transmittedPowerdBm = randomizer.nextDouble(MIN_RSSI, MAX_RSSI);
                var transmittedPower = Utils.dBmToPower(transmittedPowerdBm);
                var bssid = String.valueOf(i);
            ...
            final var estimator = new RANSACRobustRssiPositionEstimator2D(sources, fingerprint, this);
    ...
    private static double receivedPower(final double equivalentTransmittedPower, final double distance,
        ...
        final var k = Math.pow(SPEED_OF_LIGHT / (4.0 * Math.PI * RANSACRobustRssiPositionEstimator2DTest.FREQUENCY),
                pathLossExponent);
        return equivalentTransmittedPower * k / Math.pow(distance, pathLossExponent);
    }

    private static void checkLocked(final RANSACRobustRssiPositionEstimator2D estimator) {
