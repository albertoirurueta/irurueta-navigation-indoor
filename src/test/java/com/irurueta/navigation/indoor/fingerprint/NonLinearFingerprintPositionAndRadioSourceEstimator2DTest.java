...
class NonLinearFingerprintPositionAndRadioSourceEstimator2DTest implements
    ...
    void testConstructor() {
        ...
        var estimator = new NonLinearFingerprintPositionAndRadioSourceEstimator2D();
        ...
        estimator = new NonLinearFingerprintPositionAndRadioSourceEstimator2D(this);
        ...
        final var fingerprint = new RssiFingerprint<RadioSource, RssiReading<RadioSource>>(readings);

        // test constructor with located fingerprints and fingerprint
        estimator = new NonLinearFingerprintPositionAndRadioSourceEstimator2D(locatedFingerprints, fingerprint);
        ...
        assertThrows(IllegalArgumentException.class, () -> new NonLinearFingerprintPositionAndRadioSourceEstimator2D(
                null, fingerprint));
        assertThrows(IllegalArgumentException.class, () -> new NonLinearFingerprintPositionAndRadioSourceEstimator2D(
        ...
        final var initialLocatedSources = new ArrayList<RadioSourceLocated<Point2D>>();
    ...
    void testGetSetLocatedFingerprints() throws LockedException {
        final var estimator = new NonLinearFingerprintPositionAndRadioSourceEstimator2D();
    ...
    void testGetSetFingerprint() throws LockedException {
        ...
        // set new value
        final var fingerprint = new RssiFingerprint<RadioSource, RssiReading<RadioSource>>();
    ...
    void testEstimateOneRadioSourceWithoutInitialValues() throws LockedException, FingerprintEstimationException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            // create estimator
            var estimator = new NonLinearFingerprintPositionAndRadioSourceEstimator2D(locatedFingerprints, fingerprint,
                    this);
    ...
    void testEstimateOneRadioSourceWithExactInitialValues() throws LockedException, FingerprintEstimationException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            // create estimator
            final var estimator = new NonLinearFingerprintPositionAndRadioSourceEstimator2D(locatedFingerprints,
            ...
            try {
                ...
                var positionAccuracy = accuracy.getAverageAccuracyMeters();
                avgPositionAccuracy += positionAccuracy;
    ...
    void testEstimateOneRadioSourceWithApproximateInitialValues() throws LockedException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var numSources = 1;
            final var sources = new ArrayList<RadioSourceLocated<Point2D>>();
            final var initialSources = new ArrayList<RadioSourceLocated<Point2D>>();
            for (var i = 0; i < numSources; i++) {
                final var x = randomizer.nextDouble(MIN_POS, MAX_POS);
                final var y = randomizer.nextDouble(MIN_POS, MAX_POS);
                final var transmittedPowerdBm = randomizer.nextDouble(MIN_RSSI, MAX_RSSI);
                ...
                final var initialPosition = new InhomogeneousPoint2D(
                        x + positionErrorRandomizer.nextDouble(), y + positionErrorRandomizer.nextDouble());

                final var accessPoint = new WifiAccessPointWithPowerAndLocated2D("bssid" + i, FREQUENCY,
                        transmittedPowerdBm, position);
                sources.add(accessPoint);

                final var initialAccessPoint = new WifiAccessPointWithPowerAndLocated2D("bssid" + i, FREQUENCY,
    ...
    void testEstimateOneRadioSourceWithNonLocatedSources() throws LockedException, FingerprintEstimationException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            // build sources
            final var numSources = 1;
            final var locatedSources = new ArrayList<RadioSourceLocated<Point2D>>();
            ...
            for (var i = 0; i < numSources; i++) {
                ...
                final var locatedAccessPoint = new WifiAccessPointWithPowerAndLocated2D("bssid" + i, FREQUENCY,
                        transmittedPowerdBm, position);
            ...
            for (var j = 0; j < numFingerprints; j++) {
                ...
                for (final var locatedSource : locatedSources) {
                    ...
                    final var transmittedPowerdBm = ((WifiAccessPointWithPowerAndLocated2D) locatedSource)
                            .getTransmittedPower();
    ...
    void testEstimateBeacon() throws LockedException, FingerprintEstimationException, NotReadyException {
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (var i = 0; i < numSources; i++) {
                ...
                final var beacon = new BeaconLocated2D(Collections.singletonList(identifier), transmittedPowerdBm,
                        FREQUENCY, position);
                sources.add(beacon);
            }
            ...
            for (var j = 0; j < numFingerprints; j++) {
                ...
                for (final var source : sources) {
                    final var distance = source.getPosition().distanceTo(position);
                    final var transmittedPowerdBm = ((BeaconLocated2D) source).getTransmittedPower();
    ...
    void testEstimateTwoRadioSourcesWithExactInitialValues() throws LockedException, FingerprintEstimationException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (final var estimatedLocatedSource : estimatedLocatedSources) {
                ...
                final var source = sources.get(index);
                final var sourcePosition = source.getPosition();
                var sourcePositionError = estimatedSourcePosition.distanceTo(sourcePosition);
    ...
    void testEstimateWithoutEnoughFingerprintsForARadioSource() throws LockedException, NotReadyException {
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var minNearestFingerprints = 6;
            ...
            for (final var nearFingerprint : nearestFingerprints) {
                ...
                for (final var nearReading : nearReadings) {
                    if (nearReading.getSource().equals(lastSource)) {
                        ...
                        if (numReadingsLastSource >= Point2D.POINT2D_INHOMOGENEOUS_COORDINATES_LENGTH) {
    ...
    void testEstimateMultipleRadioSourcesWithoutInitialValues() throws LockedException, FingerprintEstimationException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            // build located fingerprints
            final var numFingerprints = Math.max(
