...
class WeightedKNearestNeighboursPositionSolver3DTest implements
    ...
    void testConstructor() {
        ...
        var solver = new WeightedKNearestNeighboursPositionSolver3D();
        ...
        assertThrows(IllegalArgumentException.class, () -> new WeightedKNearestNeighboursPositionSolver3D(
                null, distances));
        assertThrows(IllegalArgumentException.class, () -> new WeightedKNearestNeighboursPositionSolver3D(fingerprints,
                null));
        //noinspection unchecked
        assertThrows(IllegalArgumentException.class, () -> new WeightedKNearestNeighboursPositionSolver3D(
        ...
        solver = new WeightedKNearestNeighboursPositionSolver3D(fingerprints, distances, this);
    ...
    void testGetSetListener() throws LockedException {
        final var solver = new WeightedKNearestNeighboursPositionSolver3D();
    ...
    void testSolve1Fingerprint() throws NotReadyException, LockedException {
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var accessPointPositions = new Point3D[numAccessPoints];
            ...
            final var fingerprintsPositions = new Point3D[numFingerprints];
            final var fingerprints = new ArrayList<RssiFingerprintLocated3D<WifiAccessPoint, RssiReading<WifiAccessPoint>>>();
            for (var i = 0; i < numFingerprints; i++) {
                fingerprintsPositions[i] = new InhomogeneousPoint3D(
                        randomizer.nextDouble(MIN_POS, MAX_POS),
                        randomizer.nextDouble(MIN_POS, MAX_POS),
                        randomizer.nextDouble(MIN_POS, MAX_POS));
                var readings = new ArrayList<RssiReading<WifiAccessPoint>>();
            ...
            final var tree = new KDTree3D(Arrays.asList(fingerprintsPositions));
            ...
            final var nearestFingerprintsList =
                    new ArrayList<RssiFingerprintLocated<WifiAccessPoint, RssiReading<WifiAccessPoint>, Point3D>>();
            ...
            final var solver = new WeightedKNearestNeighboursPositionSolver3D(nearestFingerprints, nearestDistances,
                    this);
    ...
    private void findBestK(final double errorStd) throws NotReadyException, LockedException {
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (var k = 1; k < numFingerprints; k++) {
                ...
                final var solver = new WeightedKNearestNeighboursPositionSolver3D(nearestFingerprints,
                        nearestDistances);
    ...
    private void solveKFingerprintsUniformFingerprints(final int k, final double errorStd)
        ...
        for (var t = 0; t < SHORT_TIMES; t++) {
            ...
            final var fingerprintsPositionsList = new ArrayList<Point3D>();
            final var fingerprints = new ArrayList<RssiFingerprintLocated3D<WifiAccessPoint, RssiReading<WifiAccessPoint>>>();
            for (var x = MIN_POS; x < MAX_POS; x += SEPARATION_POS) {
                for (var y = MIN_POS; y < MAX_POS; y += SEPARATION_POS) {
                    for (var z = MIN_POS; z < MAX_POS; z += SEPARATION_POS) {
                        final var fingerprintPosition = new InhomogeneousPoint3D(x, y, z);
            ...
            // build tree of fingerprint positions
            final var tree = new KDTree3D(fingerprintsPositionsList);
    ...
    private static void checkLocked(final WeightedKNearestNeighboursPositionSolver3D solver) {
