...
/**
 * Linearly estimates position using located radio sources and any kind of readings at
 * unknown locations.
 * These kind of estimators can be used to determine the position of a given device by
 * getting any kind of readings at an unknown location of different radio sources whose
 * locations are known.
 *
 * @param <P> a {@link Point} type.
 */
public abstract class LinearMixedPositionEstimator<P extends Point<P>> extends MixedPositionEstimator<P> {
    ...
    private void init() {
        laterationSolverListener = new LaterationSolverListener<>() {
            ...
            public void onSolveStart(final LaterationSolver<P> solver) {
                if (listener != null) {
                    listener.onEstimateStart(LinearMixedPositionEstimator.this);
                }
            }
